...
class MoteurTPSimplifie {
    ...
    public static calculerTPS(chiffreAffaires: number, periodeFiscale: string): TPSCalculationResult {
        ...
        if (annee >= 2026) {
            return this.genererReponseErreur(chiffreAffaires, annee);
        }

        // Calcul normal pour 2025 et années antérieures
    ...
    private static genererReponseErreur(chiffreAffaires: number, annee: number): BackendEstimationFailureResponse {
        return {
            success: false,
            errors: [
                {
            ...
            context: {
                ...
                regime: chiffreAffaires > 50_000_000 ? 'Régime Réel' : 'Régime TPS',
...
//     public static calculerTPS(chiffreAffaires: number, periodeFiscale: string): GlobalEstimationInfoData {
//         const tpsCalculee = chiffreAffaires * this.TAUX_TPS;
//         const tpsBase = Math.max(tpsCalculee, this.MONTANT_MINIMUM);
//         const tpsTotale = tpsBase + this.REDEVANCE_RTB + this.CONTRIBUTION_CCI;
